    ...
    >
      <div style={{ maxWidth: '1000px', margin: '0 auto' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '64px', alignItems: 'center' }}>
          ...
          {/* Selector + thumbnails */}
          <div>
            <h2 style={{ fontFamily: 'var(--font-display)', fontSize: 'clamp(28px, 3vw, 44px)', fontWeight: 800, letterSpacing: '-0.03em', lineHeight: 1.1, color: 'var(--fg)', marginBottom: '12px' }}>
              Trusted by<br />
              ...
              {testimonials.map((t, i) => (
                <button
                  ...
                  style={{
                    ...
                    background: i === active ? 'rgba(255,85,0,0.08)' : 'rgba(13,13,13,0.04)',
                ...
                >
                  ...
                  <div>
                    <div style={{ fontSize: '13px', fontWeight: 700, color: i === active ? 'var(--fg)' : 'var(--fg-muted)', transition: 'color 0.2s' }}>{t.name}</div>
            ...
            <div style={{ display: 'flex', gap: '6px', marginTop: '24px' }}>
              {testimonials.map((_, i) => (
                <button
                  key={i}
                  onClick={() => setActive(i)}
                  style={{ width: i === active ? '24px' : '6px', height: '6px', borderRadius: '100px', background: i === active ? 'var(--accent)' : 'rgba(13,13,13,0.15)', border: 'none', cursor: 'none', transition: 'all 0.3s', padding: 0 }}
                />
              ))}
            </div>
          </div>
